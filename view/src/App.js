...
function App() {
  ...
  const stripQuotation = (obj) => {
    ...
    let replaceBracket = convertedStr
      .replace(/{/g, '{ \n\t\t')
      .replace(/}/g, ' \n\t }')
      .replace(/:/g, ': ');
          ...
          <div className="w-1/2 bg-gray-900 rounded-l-3xl relative">
            ...
            <div
              ...
              onClick={(e) => {
                ...
                e.preventDefault();
                let validCode = code;
                let isCodeJson = code.split('=');

                if (isCodeJson.length <= 1) {
                  validCode = 'const input = ' + validCode;
                }
                const types = new Translator(validCode);
                types.define();
                ...
                setOutput(
                  ` ${stripQuotation(
                    `type ${types.getVariableName()} = ${JSON.stringify(
                      typesObject
                    )}`
                  )}`
                );
              }}
              className="absolute left-96 p-4 pl-4 pr-4 text-xs font-semibold bg-white hover:bg-gray-100 cursor-pointer rounded-full"
            >
              generate
            </div>
          </div>
